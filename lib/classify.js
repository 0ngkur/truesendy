...
function buildResult({ email, localPart, domain, smtpOutcome, isCatchAllDomain, hadMx, mxHosts }) {
  ...
  if (smtpOutcome.result === 'accepted') {
    if (isCatchAllDomain) {
      ...
      return finalize('catch_all', 'catch_all_domain', false, 60);
    }
    if (roleBased) {
      // Role accounts (info@, admin@) are deliverable but high-risk
  ...
  if (smtpOutcome.result === 'rejected') {
    ...
    if (rejType === 'sender_blocked') {
      return finalize('unknown', 'sender_ip_blocked', false, 25);
    }
  ...
  // ── Connection failed — MX exists but server unreachable ─────────────────────
  // For ANTI-PROBE PROVIDERS (Microsoft 365, Google Workspace, Mimecast, Proofpoint, etc.):
  //   These providers INTENTIONALLY block external SMTP probes as an anti-spam measure.
  //   The domain is real, MX records exist — the email is very likely valid.
