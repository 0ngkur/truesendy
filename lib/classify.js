...
function buildResult({ email, localPart, domain, smtpOutcome, isCatchAllDomain, hadMx, mxHosts }) {
  ...
  // Store MX hostnames for Reon-compatible result output
  ...
  function finalize(status, reasonCode, safeToSend, score) {
    return {
      ...
      flags: {
        disposable,
        roleBased,
        // catchAll flag must be true when status is catch_all (regardless of SMTP probe result)
        // This ensures is_catch_all column in CSV always matches the status column
