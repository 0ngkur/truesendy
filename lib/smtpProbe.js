 ...
 */
async function probeCatchAll(mxHosts, domain, heloDomain, timeoutMs, prober, sleep) {
    ...
    // Greylisting retry: 4xx temp_fail often succeeds on 2nd attempt (mirrors real probe).
    // Previously the catch-all probe had NO retry, so catch-all domains that greylist
    // the probe were missed — the root cause of TrueSendy vs Reoon catch-all divergence.
    if (outcome.result === 'temp_fail') {
        await sleep(5000); // 5s wait — standard greylisting window
        const retryOutcome = await prober(mxHosts, '', fakeAddr, heloDomain, timeoutMs);
...
async function checkMailbox(mxHosts, domain, fullEmail, opts = {}) {
    ...
    const timeoutMs  = 15000; // 15s — matches Reoon's reliability for slow corporate servers
    ...
    if (realOutcome.result === 'accepted') {
        ...
        const fakeOutcome = await probeCatchAll(mxHosts, domain, heloDomain, timeoutMs, prober, sleep);
        ...
        // connection_failed, sender_rejected, temp_fail → still try catch-all probe
        // Many corporate gateways block the real probe but accept the fake address
        // (catch-all behaviour behind a firewall). This catches ~38 missed catch-all.
        const fakeOutcome = await probeCatchAll(mxHosts, domain, heloDomain, timeoutMs, prober, sleep);
