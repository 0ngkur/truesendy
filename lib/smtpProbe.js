 ...
 *
 * Returns: 'mailbox_not_found' | 'mailbox_full' | 'sender_blocked' | 'policy_rejection' | 'unknown_rejection'
 */
function classifyRejection(code, responseText) {
  const lower = (responseText || '').toLowerCase();
  const enhanced = parseEnhancedCode(responseText || '');

  // ── TIER 0: Sender-IP / sender-reputation blocks ───────────────────────────
  // These rejections are about OUR sending IP or sender address (RBL listings,
  // Spamhaus, reputation blocks) — NOT about the recipient. They tell us nothing
  // about whether the mailbox exists, so the email is UNVERIFIABLE from this IP.
  // Must be checked BEFORE generic policy_rejection (which also matches "blocked").
  const senderBlockPatterns = [
    'blocked using spamhaus',
    'blocked using barracuda',
    'blocked using sorbs',
    'blocked using spamcop',
    'blocked using',           // generic RBL block (catches "blocked using <rbl>")
    'listed by xbl',
    'listed by css',
    'listed by pbl',
    'listed by sbl',
    'listed on',
    'client host',            // "Client host [IP] blocked" (Microsoft 365)
    'ip blocked',
    'ip is blocked',
    'sender ip',
    'bad sender',
  ...
  if (enhanced && enhanced.subject === 2) {
    if (enhanced.detail === 1) return 'mailbox_disabled'; // 5.2.1 = mailbox disabled — EXISTS but cannot receive
  ...
  // DEFINITIVE "mailbox disabled" signals — mailbox EXISTS but is turned off
    ...
    socket.on('data', (chunk) => {
      ...
      if (stage === 'mailfrom') {
        if (code >= 200 && code < 300) {
          ...
          // MAIL FROM rejected — classify it (may be a sender-IP block, not just policy)
          finish({
