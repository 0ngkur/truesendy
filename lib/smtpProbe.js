...
async function checkMailbox(mxHosts, domain, fullEmail) {
    ...
    let usedSender    = nullSender;   // Track which sender worked for catch-all probes
    let realOutcome   = await probeAcrossMxHosts(mxHosts, nullSender, fullEmail, heloDomain, timeoutMs);

    // Strategy 2: If null sender failed at MAIL FROM, fall back to a domain address.
    if (realOutcome.result === 'sender_rejected') {
        usedSender = domainSender;
    ...
    if (realOutcome.result === 'accepted') {
        ...
        // IMPORTANT: Use the SAME sender that worked for the real probe
        const fakeAddress = buildRandomProbeAddress(domain);
        const fakeOutcome = await probeAcrossMxHosts(mxHosts, usedSender, fakeAddress, heloDomain, timeoutMs);
        ...
        if (realOutcome.rejectionType !== 'mailbox_not_found' &&
            ...
            const fakeAddress = buildRandomProbeAddress(domain);
            // Use the SAME sender that worked for the real probe
            const fakeOutcome = await probeAcrossMxHosts(mxHosts, usedSender, fakeAddress, heloDomain, timeoutMs);
            isCatchAll = fakeOutcome.result === 'accepted';
        }
        ...
        // Try both senders for the fake probe
        const fake1 = buildRandomProbeAddress(domain);
        let fake1Outcome = await probeAcrossMxHosts(mxHosts, nullSender, fake1, heloDomain, timeoutMs);
        if (fake1Outcome.result === 'sender_rejected') {
            fake1Outcome = await probeAcrossMxHosts(mxHosts, domainSender, fake1, heloDomain, timeoutMs);
        }
