...
async function checkMailbox(mxHosts, domain, fullEmail, opts = {}) {
    // HELO must be a domain that A-records back to THIS server's IP, so strict mail
    // gateways (Mimecast/Proofpoint/Barracuda) pass forward-confirmed reverse DNS
    // and don't drop the connection. The apex truesendy.com already resolves to us;
    // verify.truesendy.com did NOT resolve (ENOTFOUND) → gateways rejected it.
    // (For max effect, also set a PTR on 72.60.195.145 -> truesendy.com later.)
    const heloDomain = 'truesendy.com';
    const timeoutMs  = 8000;  // 8s — balance: fast failure on dead servers vs giving slow ones a chance
    const prober = opts.prober || probeAcrossMxHosts;
    const sleep  = opts.sleep  || (ms => new Promise(r => setTimeout(r, ms)));

    // Strategy 1: Null sender (RFC-standard, bypasses reputation blocks)
    let realOutcome = await prober(mxHosts, '', fullEmail, heloDomain, timeoutMs);

    // Strategy 2: Fall back to domain sender if null sender rejected
    if (realOutcome.result === 'sender_rejected') {
        realOutcome = await prober(mxHosts, 'verify@truesendy.com', fullEmail, heloDomain, timeoutMs);
    }

    // Strategy 3: Greylisting retry — 4xx temp_fail often succeeds on 2nd attempt
    if (realOutcome.result === 'temp_fail') {
        await sleep(5000); // 5s wait — standard greylisting window
        const retryOutcome = await prober(mxHosts, '', fullEmail, heloDomain, timeoutMs);
        // Only use retry result if it's more definitive than temp_fail
        if (retryOutcome.result === 'accepted' || retryOutcome.result === 'rejected') {
            realOutcome = retryOutcome;
        }
    }

    // If the caller already knows this domain is NOT catch-all (per-domain cache in
    // verifier.js), skip the fake probe entirely — one fewer SMTP conversation per
    // email on large lists where many emails share a domain.
    ...
    // ── ALWAYS attempt catch-all detection ─────────────────────────────────────
    // Many catch-all servers block the real probe (sender_blocked, connection_failed)
    // while still accepting a fake address. Probing catch-all in all cases catches
    // ~38 more catch-all emails.
    if (realOutcome.result === 'accepted') {
        // Standard catch-all check: probe accepted → test fake address
        const fakeOutcome = await probeCatchAll(mxHosts, domain, heloDomain, timeoutMs, prober);
        isCatchAll = fakeOutcome.result === 'accepted';
    } else if (realOutcome.result === 'rejected') {
        // Rejected probe → still check catch-all (some servers reject our IP but accept everything else)
        ...
        // Connection succeeded but the real probe was rejected/greylisted → still
        // try catch-all (some servers reject our sender but accept a fake address).
        const fakeOutcome = await probeCatchAll(mxHosts, domain, heloDomain, timeoutMs, prober);
    ...
    // else: connection_failed → SKIP the fake probe. If the connection itself
    // failed, the fake-address probe will fail too — running it just burns a
    // second 8s timeout per blocked server (the main cause of slow runs on
    // anti-probe-heavy lists). isCatchAll stays false.
